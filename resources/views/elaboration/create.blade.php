    ...
    <script>
        ...
        function addSupplies(id) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    let table = $('#suppliesDetail').DataTable();
                    let elements = table.rows().data();
                    let count = table.rows().count();
                    let flag = false;
                    for (let i = 0; i < count; i++) {
                        if (elements[i][0] == data.id) {
                            flag = true;
                            break;
                        }
                    }
                    if (flag) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'El insumo ya se encuentra en la lista',
                        })
                    } else {
        ...
        $('#insumoModalSave').click(function() {
            ...
            table.row.add([
                `${$insumo_id}`,
                $insumo_name,
                `${$insumo_cantidad} <input type="text" name="insumo_cantidad[]" value="${$insumo_cantidad}" hidden> <input type="text" name="insumo_id[]" value="${$insumo_id}" hidden>`,
