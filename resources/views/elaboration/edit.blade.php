            ...
            <div class="col-md-12">
                ...
                <div class="card card-default">
                    <div class="card-header">
                        <span class="card-title">Editar Elaboration</span>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('elaborations.update1', ['id'=> $elaboration->id]) }}" role="form"
                            id="form">
                            @csrf
                            <div class="row">
                                <div class="col-md-6">
                                        ...
                                        <select name="products" id="products" class="form-control">
                                            ...
                                            @foreach ($products as $product)
                                                <option value="{{ $product->id }}"
                                                    @if ($product->id == $elaboration->product_id) selected @endif>{{ $product->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group{{ $errors->has('cuantity') ? ' has-error' : '' }}">
                                        ...
                                        {!! Form::number('cuantity', $elaboration->cuantity, ['class' => 'form-control', 'required' => 'required']) !!}
                                                ...
                                                <div class="row">
                                                    <div class="col-sm-12">
                                                        <table id="supplies"
                                                            ...
                                                            aria-describedby="supplies_info">
                                                            <thead>
                                                                <tr>
                                                                    <th>
                                                                        #
                                                                ...
                                                                @foreach ($supplies as $supply)
                                                                    <tr>
                                                                        ...
                                                                        <td class="text-center">
                                                                            <button id="add-supplies" type="button"
                                                                                onclick="addSupplies( {{ $supply->id }} )"
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            $.ajax({
                url: "{{ route('detail.elaboration', 'id') }}".replace('id', {{ $elaboration->id }}),
                type: 'GET',
                success: function(data) {
                    data.forEach(element => {
                        $('#suppliesDetail').DataTable().row.add([
                            `${element.supply_id}`,
                            element.name,
                            `${element.quantity} `,
                            `
                    <button type="button" class="btn btn-danger btn-sm" onclick="deleteSupplies(${element.supply_id})"><i class="fas fa-trash"></i></button>
                    <button type="button" class="btn btn-warning btn-sm" onclick="editSupplies(${element.supply_id})"><i class="fas fa-edit"></i></button>
                    <input type="text" name="insumo_cantidad[]" value="${element.quantity}" hidden> <input type="text" name="insumo_id[]" value="${element.supply_id}" hidden>
                `
                        ]).draw(false);
                    });
                },
        ...
        $('#insumoModalSave').click(function() {
            ...
            table.row.add([
                ...
                $insumo_name,
                `${$insumo_cantidad}`,
                `<button type="button" class="btn btn-danger btn-sm" onclick="deleteSupplies(${$insumo_id})"><i class="fas fa-trash"></i></button> <button type="button" class="btn btn-warning btn-sm" onclick="editSupplies(${$insumo_id})"><i class="fas fa-edit"></i></button>
                <input type="text" name="insumo_cantidad[]" value="${$insumo_cantidad}" hidden>  <input type="text" name="insumo_id[]" value="${$insumo_id}" hidden>
                `
            ]).draw(false);
        ...
        $('.btn-close').click(function() {
            $cantidad = $('#insumo_cantidad').val();
            $stock = $('#insumo_stock').val();
            // retorno todo
            $('#insumoModal').modal('hide');
        });
                ...
                if (elements[i][0] == id) {
                    for (let y = 0; y < tableinsumo.rows().count(); y++) {
                        if (elementsinsumos[y][0] == elements[i][0]) {
                            var stock = parseInt(elementsinsumos[y][2]) + parseInt(elements[i][2]);
        ...
        function editSupplies(id) {
            var table = $('#suppliesDetail').DataTable();
            var elements = table.rows().data();
            var tableinsumo = $('#supplies').DataTable(); // tabla de insumos
            var elementsinsumos = tableinsumo.rows().data();
            for (let i = 0; i < table.rows().count(); i++) {
                if (elements[i][0] == id) {
                    for (let y = 0; y < tableinsumo.rows().count(); y++) {
                        if (elementsinsumos[y][0] == elements[i][0]) {
                            var stock = parseInt(elementsinsumos[y][2]) + parseInt(elements[i][2]);
                            tableinsumo.cell(y, 2).data(stock).draw(false);
                            $('#insumo_cantidad').val(elements[i][2]);
                        }
                    }
                    table.row(i).remove().draw(false)
                }
